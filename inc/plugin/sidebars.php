	    ...
	    if(is_array($sidebars)){
			foreach($sidebars as $sidebar){
				...
				register_sidebar(array(
					...
					'after_widget' => '</div>',
					'before_title' => '<h3 class="regular-title row">',
					'after_title' => '</h2>',
		    	));
			}
		}
	...
	function admin_page(){
		...
		<div class="wrap">
			...
			<table class="widefat page" id="sbg_table" style="width:600px;">
				<tr>
					<th><?php _e('Sidebar Name','sosthemes') ?></th>
					<th><?php _e('CSS clas','sosthemes') ?>s</th>
					<th><?php _e('Remove','sosthemes') ?></th>
				</tr>
				<?php
				$sidebars = sidebar_generator::get_sidebars();
				...
				<tr class="<?php echo $alt?>">
					...
					<td><a href="javascript:void(0);" onclick="return remove_sidebar_link('<?php echo $sidebar; ?>',<?php echo $cnt+1; ?>);" title="Remove this sidebar"><?php _e('Remove','sosthemes') ?></a></td>
				</tr>
				<?php
						$cnt++;
					}
					...
					?>
					<tr>
						<td colspan="3"><?php _e('No Sidebars defined','sosthemes') ?></td>
					</tr>
					<?php
				}
				?>
            ...
            <div class="add_sidebar">
				<a href="javascript:void(0);" onclick="return add_sidebar_link()" title="Add a sidebar" class="button-primary"><?php _e('+ Add New Sidebar','sosthemes') ?></a>
			</div>
		</div>
		<?php
	}
		...
		<div id="sbg_box" class="postbox " >
			...
			<div class="inside">
				<div class="sbg_container">
					...
					<p><?php _e('Please select the sidebar you would like to display on this page. <strong>Note:</strong> You must first create the sidebar under Appearance > Sidebars.','sosthemes') ?>
					</p>
					<ul>
					<?php 
					...
					//var_dump($wp_registered_sidebars);		
						for($i=0;$i<1;$i++){ ?>
							<li>
							<select id="sos_sb_status" name="sidebar_generator[<?php echo $i?>]" style="display: none;">
								<option value="0"<?php if($selected_sidebar[$i] == ''){ echo " selected";} ?>><?php _e('Default Sidebars','sosthemes') ?></option>
							<?php
							...
							<select id="sos_sb_status2" name="sidebar_generator_replacement[<?php echo $i?>]">
								<option value="0"<?php if($selected_sidebar_replacement[$i] == ''){ echo " selected";} ?>><?php _e('None','sosthemes') ?></option>
							<?php
